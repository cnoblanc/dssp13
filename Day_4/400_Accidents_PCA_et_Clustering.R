...
PCA_Carac <- PCA(caracteristiques_quali, graph = FALSE)
str(PCA_Carac)

C_Carac <- cov(as.matrix(caracteristiques_quali))
E_Carac <- eigen(C_Carac)
U_Carac <- E_Carac$vectors
Lambda_Carac <- E_Carac$values

PCA_Carac_2 <- as.matrix(caracteristiques_quali) %*% U_Carac
...
t(PCA_Carac$eig)
# Les 2 premiers axes représentent 11,6 % !! c'est trop peu.
...
fviz_pca_ind(PCA_Carac, col.ind="cos2")  # ça prends un temps fou à tracer car trop de points.
