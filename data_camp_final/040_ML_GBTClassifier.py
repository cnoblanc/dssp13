...
fileName_train=HDFS_base_path+"/train_features_110.parquet"
fileName_valid=HDFS_base_path+"/valid_features_110.parquet"
...
(train,test)=dataDF.rdd.randomSplit([0.1,0.1],seed=42)
...
max_iterations=10
col_name="html"
reg_html=GBTClassifier(featuresCol=tfidf_col_name,labelCol=col_name,predictionCol=col_name+"_pred",maxIter=max_iterations)
Model_html = reg_html.fit(train.toDF())
print "################ Start fitting the model : jquery"
col_name="jquery"
reg_jquery=GBTClassifier(featuresCol=tfidf_col_name,labelCol=col_name,predictionCol=col_name+"_pred",maxIter=max_iterations)
Model_jquery = reg_jquery.fit(train.toDF())
print "################ Start fitting the model : css"
col_name="css"
reg_css=GBTClassifier(featuresCol=tfidf_col_name,labelCol=col_name,predictionCol=col_name+"_pred",maxIter=max_iterations)
Model_css = reg_css.fit(train.toDF())
print "################ Start fitting the model : javascript"
col_name="javascript"
reg_javascript=GBTClassifier(featuresCol=tfidf_col_name,labelCol=col_name,predictionCol=col_name+"_pred",maxIter=max_iterations)
...
def apply_model(DF):
    ...
    result_html=Model_html.transform(DF).select(col("id").alias("html_id"),'html_pred')
    result_jquery=Model_jquery.transform(DF).select(col("id").alias("jquery_id"),'jquery_pred')
    result_css=Model_css.transform(DF).select(col("id").alias("css_id"),'css_pred')
    result_javascript=Model_javascript.transform(DF).select(col("id").alias("javascript_id"),'javascript_pred')
...
col_name="html"
evaluator_html = BinaryClassificationEvaluator(rawPredictionCol=col_name+"_pred",labelCol=col_name,metricName=metricName,)
col_name="jquery"
evaluator_jquery = BinaryClassificationEvaluator(rawPredictionCol=col_name+"_pred",labelCol=col_name,metricName=metricName,)
col_name="css"
evaluator_css = BinaryClassificationEvaluator(rawPredictionCol=col_name+"_pred",labelCol=col_name,metricName=metricName,)
col_name="javascript"
evaluator_javascript = BinaryClassificationEvaluator(rawPredictionCol=col_name+"_pred",labelCol=col_name,metricName=metricName,)
