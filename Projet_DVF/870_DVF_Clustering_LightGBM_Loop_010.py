...
print("---------------------------------------")
print("Hierarchical clustering : pour déterminer le nombre de clusters à utiliser")
print("---------------------------------------")
...
t0 = time()
cluster_linkage=shc.linkage(X_train_transformed, method=cluster_distance) 
print("---------------------------------------")
print("Ward linkages Done in : %0.3fs" % (time() - t0))

cluster_max_distance=max(cluster_linkage[:,2])
color_threshold=0.7*cluster_max_distance

plt.figure(figsize=(14, 6))
plt.title('Hierarchical Clustering Dendrogram (%s, Max distance=%.2f , blue: dist >= %.2f)' % (cluster_distance,cluster_max_distance,color_threshold))
dend = shc.dendrogram(cluster_linkage
                      #,orientation='left'
                      ,truncate_mode='level',p=5)
plt.plot([0, 1000], [245, 245], 'k-', color = 'lightblue')
plt.plot([0, 1000], [415, 415], 'k-', color = 'lightblue')
plt.xticks(fontsize = 10)
plt.xlabel("Number of points in node (or index of point if no parenthesis)",fontsize = 10)
plt.ylabel("%s distance between points" % (cluster_distance),fontsize = 10)
plt.show()
...
def generateModelScores(X_train,y_train,X_test,y_test):
    ...
    if X_train.shape[0] > folds_num :
        ...
        cross_val_scores=mean_absolute_error(y_train, y_train_pred, multioutput='raw_values')
...
columns = ['cluster_cnt','init','inertia','cluster_time','crossval_mae','test_mae','test_mape','test_rmse','models_time']
index=range(max_clusters)
df_cluster_results = pd.DataFrame(index=index, columns=columns)
df_cluster_results[['inertia','cluster_cnt','cluster_time','crossval_mae','test_mae','test_mape','test_rmse','models_time']]=0.0
...
for i in range(max_clusters):
    ...
    columns = ['cluster','val_mae','mae', 'mape','rmse','total_time','train_len','test_len']
    index=range(clusters_cnt)
    df_results = pd.DataFrame(index=index, columns=columns)
    df_results[['cluster','val_mae','mae', 'mape','rmse','total_time','train_len','test_len']]=0.0
    
    for clust in range(clusters_cnt):
        print("---------------------------------------")
        ...
        df_results.iloc[clust]['val_mae']=val_mae
    ...
    df_cluster_results.iloc[i]['crossval_mae']=df_results['weighted_val_mae'].sum()/df_results['train_len'].sum()
    ...
    print("Cross-Validation Accuracy: %0.2f" % (df_cluster_results.iloc[i]['crossval_mae']))
...
parquet_fileName=base_path+"keras_models_saved/df_results_Clusters_LightGBM_GridSearch_003"
